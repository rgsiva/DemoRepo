...
public class DriverFactory {
	...
	/**
	 * Function to move to the new window
	 * 
	 * @param windowTitle The number of seconds to wait while checking for the alert
	 * 
	 * @return WebDriver indicating whether an alert is present
	 */
	public static WebDriver getHandleToWindow(WebDriver driver, String windowTitle) {

		WebDriver hsqWindow = null;
		Set<String> windowIterator = driver.getWindowHandles();

		for (String s : windowIterator) {
			String windowHandle = s;
			hsqWindow = driver.switchTo().window(windowHandle);

			if (hsqWindow.getTitle().equals(windowTitle)) {
				return hsqWindow;
			}
